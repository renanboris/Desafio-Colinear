...
- Ingestão: Lê múltiplos arquivos CSV contábeis com layouts hierárquicos (Pai: Conta -> Filho: Lançamentos).

- Parsing Inteligente:

- Detecta automaticamente a posição das colunas (Data, Histórico, Débito, Crédito) sem depender de índices fixos.
...
- Formata valores numéricos e define tipo (D/C).

*Consolidação: Gera um dataset único, desduplicado e formatado conforme a regra de negócio: [descrição limpa] valor [0.00] [d/c]*

##🧠 Decisões Técnicas

Biblioteca CSV vs Pandas na leitura: Optei por usar a biblioteca nativa csv para o parsing linha-a-linha.
...
*Detecção Dinâmica de Colunas:* Em vez de usar "números mágicos" (ex: coluna 5), o script varre o cabeçalho procurando por palavras-chave ("Histórico", "Débito"). Isso torna o código resiliente a mudanças de layout.
...
*Variação de Layout:* Arquivos onde a coluna "Histórico" mudava de posição. Resolvido com a lógica de header_found e busca por índices baseada em strings.
